...
export default function Cadastro() {
 ...
 <Input id="corem" placeholder="Ex: COREN-XX123456" value={formData.corem || ''} onChange={(e) => handleChange('corem', e.target.value)} />
 </div>
 <div className="space-y-2">
 <Label htmlFor="backgroundCheckNotes">Observações sobre antecedentes</Label>
 <Textarea id="backgroundCheckNotes" placeholder="Descreva se possui antecedentes ou informações relevantes" value={formData.backgroundCheckNotes || ''} onChange={(e) => handleChange('backgroundCheckNotes', e.target.value)} />
 </div>
 </div>

 <div className="space-y-2">
 <Label htmlFor="profilePhoto">Foto de Perfil (JPG/PNG)</Label>
 <Input id="profilePhoto" type="file" accept="image/png, image/jpeg, image/jpg" onChange={handleProfilePhotoChange} />
 {profilePhotoPreview && (
 <img src={profilePhotoPreview} alt="preview" className="mt-2 w-32 h-32 object-cover rounded" />
 )}
 </div>

 <div className="space-y-2">
 <Label htmlFor="background_check_file">Upload - Antecedente criminal (PDF)</Label>
 <Input id="background_check_file" type="file" accept="application/pdf" onChange={handleFileChange} />
 <p className="text-xs text-muted-foreground">O arquivo será disponibilizado aos contratantes após aprovação do perfil.</p>
 </div>

 <div className="space-y-2">
 <Label htmlFor="certificates">Upload - Certificados (JPG/PNG) — múltiplos</Label>
 <Input id="certificates" type="file" accept="image/png, image/jpeg, image/jpg" multiple onChange={handleCertificatesChange} />
 <p className="text-xs text-muted-foreground">Adicione certificados em JPG ou PNG (ex.: cursos, especializações). Máx. recomendado:10 arquivos.</p>
 {certificateFiles && certificateFiles.length >0 && (
 <div className="mt-2 text-xs text-muted-foreground">
 {certificateFiles.map((f, i) => <div key={i}>• {f.name}</div>)}
 </div>
 )}
 </div>

 <div className="space-y-2">
 <Label htmlFor="video">Vídeo de Apresentação (MP4)</Label>
 <Input id="video" type="file" accept="video/mp4" onChange={handleVideoChange} />
 {videoPreview && (
 ...
 <Input id="city" placeholder="Sua cidade" value={formData.city} onChange={(e) => handleChange('city', e.target.value)} required />
 </div>
 </div>

 <Button type="button" className="w-full mt-4" onClick={() => setStep(2)} disabled={!formData.name || !formData.birthDate || !formData.sex || !formData.profession || !formData.state || !formData.city}>
 Continuar
 ...
 <Input id="whatsapp" placeholder="(11)99999-9999" value={formData.whatsapp} onChange={(e) => handleChange('whatsapp', e.target.value)} required />
 </div>

 <div className="space-y-2">
 <Label htmlFor="email">Email *</Label>
 <Input id="email" type="email" placeholder="seu@email.com" value={formData.email} onChange={(e) => handleChange('email', e.target.value)} required />
 </div>
 ...
 <Input id="confirmPassword" type="password" placeholder="••••••" value={formData.confirmPassword} onChange={(e) => handleChange('confirmPassword', e.target.value)} required />
 </div>
 </div>

 <p className="text-sm text-muted-foreground">Ao criar sua conta, você concorda com nossos <a href="/termos-profissionais" className="text-primary hover:underline">Termos de Uso</a> e <a href="/privacidade" className="text-primary hover:underline">Política de Privacidade</a>.</p>
 ...
 <span className="text-sm text-muted-foreground">Já tem uma conta? <a href="/login" className="text-primary hover:underline font-medium">Entrar</a></span>
 </div>
 </CardContent>
 </Card>
 </div>
